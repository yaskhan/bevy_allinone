...
pub fn update_camera_shake(
    ...
    mut camera_query: Query<(&CameraController, &mut CameraState, Entity)>,
    ...
    for request in shakes_queue.0.drain(..) {
        if let Some((_, _, camera_ent)) = camera_query.iter_mut().next() {
            // Here we would lookup 'request.name' in a preset database
            // For now, spawn a generic instance based on request
            commands.spawn(CameraShakeInstance {
                camera_entity: Some(camera_ent),
                name: request.name,
                intensity: request.intensity,
                duration: request.duration.unwrap_or(0.5),
                pos_amount: Vec3::splat(0.1 * request.intensity),
                rot_amount: Vec3::splat(3.0 * request.intensity),
                ..default()
            });
        }
    }

    // 2. Accumulate all active shakes
    // Map of camera entities to their total offsets
    ...
    for (ent, mut shake) in shakes_instances.iter_mut() {
        ...
        // Calculate target positions using Sine waves (Port from headBob.cs / movementBob)
        let phase = elapsed;
        
        let target_pos_x = (phase * shake.pos_speed.x).sin() * shake.pos_amount.x;
        let target_pos_y = (phase * shake.pos_speed.y).sin() * shake.pos_amount.y;
        let target_pos_z = (phase * shake.pos_speed.z).cos() * shake.pos_amount.z;
        let target_pos = Vec3::new(target_pos_x, target_pos_y, target_pos_z);

        let target_rot_x = (phase * shake.rot_speed.x).sin() * shake.rot_amount.x;
        let target_rot_y = (phase * shake.rot_speed.y).sin() * shake.rot_amount.y;
        let target_rot_z = (phase * shake.rot_speed.z).cos() * shake.rot_amount.z;
        let target_rot = Vec3::new(target_rot_x, target_rot_y, target_rot_z);

        // Apply smoothing and intensity reduction
    ...
    // 3. Apply breathing noise + aggregate shakes to CameraState
    for (_camera, mut state, cam_ent) in camera_query.iter_mut() {
        // Breathing noise (legacy logic)
        let t = elapsed * 2.0;
        ...
        // Combine with shakes for this specific camera
        if let Some((_total_pos, total_rot)) = camera_offsets.get(&cam_ent) {
            state.noise_offset.x += total_rot.y * 0.1; // Yaw shake
            state.noise_offset.y += total_rot.x * 0.1; // Pitch shake
        }
    }
}
