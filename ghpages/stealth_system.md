...
fn update_custom_hide_states(
    mut query: Query<(&StealthController, &mut StealthState, &Transform)>
) {
...
fn detect_custom_cover_types(
    mut query: Query<(&StealthController, &mut CoverDetection, &Transform)>,
    foliage_query: Query<&Transform, With<FoliageMarker>>,
) {
...
fn update_environmental_detection(
    mut query: Query<(&mut VisibilityMeter, &EnvironmentalDetection, &Transform)>,
...
fn update_custom_camera_modes(
    mut query: Query<(&StealthState, &CustomCameraMode, &mut Transform)>,
    camera_query: Query<&mut Transform, (With<Camera>, Without<StealthState>)>,
) {
